...
template <typename T>
static void onSourcePropertiesSet(QString name, const T &list)
{
	int index = 0;
	for (auto &iter : list)
	{
		CController::GetInstance().WriteToLogFile("SourceList index: %d source name: %s",
			index++, QT_TO_UTF8(iter->getSourceName()));
		if (iter->getSourceName() == name)
		{
			CController::GetInstance().WriteToLogFile("Set source of name: %s", QT_TO_UTF8(iter->getSourceName()));
			iter->Load();
			return;
		}
	}
...
template <typename T>
static void onSourceRemoved(QString name, T &list)
{
	int index = -1;
	bool flag = false;
	for (auto &iter : list)
	{
		...
		{
			flag = true;
			CController::GetInstance().WriteToLogFile("Del source from sourceList: %s", QT_TO_UTF8(name));
			iter->Release();
			iter->deleteLater();
			break;
		}
...
#define HEARTBEAT_VALIDATION_INTERVAL 30000

OBSBasic::OBSBasic(QWidget *parent)
	: OBSMainWindow(parent),
	ui(new Ui::OBSBasic)
	, m_pBarrageAssistantDlg(new BarrageAssistantDlg(this))
	, m_pSystemSettingsDlg(NULL)
	, m_pTQLiveAboutDlg(NULL)
	, m_pRoomManageAssistantDlg(NULL)
	, m_pGroupChatSettingsDlg(NULL)
	, m_pRedEnvelopeDlg(NULL)
	, m_nGroupNum(-1)
	, m_pExitAppTipsDlg(NULL)
	, m_pSystemTray(NULL)
	, m_pBroadcastReminderDlg(NULL)
	, m_pOffcastStatisticsDlg(new OffcastStatisticsDlg(this))
	, m_eExitType(kNormalExit)
	, m_nLiveAbortTpye(NORMAL_TERMINATED)
	, m_nBarrageMsgTotal(0)
	, m_nGeneralUserTotal(0)
	, m_nVipUserTotal(0)
	, m_nRealBarrageMsgTotal(0)
	, m_pSoundEffectMenu(NULL)
	, m_pAudioControlWin(new AudioControlWindow)
	, m_nLiveRecordID(0)
{
	...
	ui->scenes->setViewMode(QListView::IconMode);
	ui->scenes->setSpacing(0);
	ui->scenes->setResizeMode(QListWidget::Adjust);
	ui->scenes->setMovement(QListWidget::Static);

	ui->wd_display_user_info->installEventFilter(this);
	ui->preview->installEventFilter(this);
	ui->wd_control_panel->installEventFilter(this);
	ui->sourcesFrame->installEventFilter(this);
	ui->materialFrame->installEventFilter(this);
	ui->scenes->installEventFilter(this);

	// 页面效果定制
	ui->menubar->hide();
	ui->transitionsContainer->hide();
	ui->modeSwitch->hide();
	ui->exitButton->hide();
	ui->settingsButton->hide();
	ui->sourcesToolbar->hide();
	ui->scenesToolbar->hide();

	m_eDisplayState = kNormalState;

	connect(ui->pb_minimize, SIGNAL(clicked()), this, SLOT(showMinimized()));
	connect(ui->pb_maximize, SIGNAL(clicked()), this, SLOT(slotShowMaximizedOrNormal()));
	connect(ui->pb_close, SIGNAL(clicked()), this, SLOT(onExitApplication()));
	connect(ui->pb_modify_live_room_name, SIGNAL(clicked()), this, SLOT(slotModifyLiveRoomName()));
	connect(ui->pb_modify_save, SIGNAL(clicked()), this, SLOT(slotModificationSave()));
	connect(ui->pb_modify_cancel, SIGNAL(clicked()), this, SLOT(slotModificationCancel()));
	connect(ui->pb_live_content, SIGNAL(clicked()), this, SLOT(slotModifyLiveContent()));
	connect(ui->statusbar, &OBSBasicStatusBar::onSigUpdateRecordTime, this, [&](QString time) {
		ui->label_recordTime->setText(time);
	});
	connect(ui->statusbar, &OBSBasicStatusBar::onSigUpdateLiveTime, this, [&](QString time) {
		ui->label_liveTime->setText(time);
	});

	connect(ui->pb_bullet_screen, &QPushButton::clicked, this, [&]() {
		m_pBarrageAssistantDlg->move(this->pos().x() + this->width() + 2, this->pos().y());
		m_pBarrageAssistantDlg->setHidden(!m_pBarrageAssistantDlg->isHidden());
	});
	connect(m_pBarrageAssistantDlg, &BarrageAssistantDlg::onSigMicHotKeyTrigger, [&]() {
		m_pAudioControlWin->SetMicHotKey(UserConfig::instance()->getMicHotKey());
	});
	connect(m_pBarrageAssistantDlg, SIGNAL(onSigLiveHotKeyTrigger()), this, SLOT(on_streamButton_clicked()));

	connect(ui->pb_settings_menu, &QPushButton::clicked, this, [&]() {
		if (m_pSystemSettingsDlg == NULL)
		{
			m_pSystemSettingsDlg = new SystemSettingsDlg(this);
			connect(m_pSystemSettingsDlg, SIGNAL(onSigShowAboutDlg()), this, SLOT(onShowAboutDlg()));
			connect(m_pSystemSettingsDlg, SIGNAL(onSigShowSettingsIniDlg()), this, SLOT(onShowSettingsIniDlg()));
			connect(m_pSystemSettingsDlg, SIGNAL(onSigShowLogoutDlg()), this, SLOT(onShowLogoutDlg()));
			connect(m_pSystemSettingsDlg, SIGNAL(onSigShowFeedbackDlg()), this, SLOT(onShowFeedbackDlg()));
		}

		QPoint p = ui->pb_settings_menu->pos();
		int posX = p.x() + this->pos().x() - m_pSystemSettingsDlg->width() + ui->pb_settings_menu->width();
		int posY = p.y() + this->pos().y() + ui->pb_settings_menu->height() + 9;
		m_pSystemSettingsDlg->move(posX, posY);
		m_pSystemSettingsDlg->show();
	});

	connect(ui->pb_room_management, &QPushButton::clicked, this, [&]() {
		if (m_pRoomManageAssistantDlg == NULL)
		{
			m_pRoomManageAssistantDlg = new RoomManageAssistantDlg(this);
		}
		m_pRoomManageAssistantDlg->InitData();
	});

	if (UserConfig::instance()->getGroupId() == -1)
	{
		m_stLiveContentInfo.nGroupID = g_dataCentre.stUserInformation.nGeneralizeGroupID;
	}
	else
	{
		m_stLiveContentInfo.nGroupID = UserConfig::instance()->getGroupId();
	}
	UserConfig::instance()->setGroupNumber(g_dataCentre.stUserInformation.qsGeneralizeGroupNumber);

	connect(ui->pb_group_settings, &QPushButton::clicked, this, [&]() {
		if (m_pGroupChatSettingsDlg == NULL)
		{
			m_pGroupChatSettingsDlg = new GroupChatSettingsDlg(this);
			connect(m_pGroupChatSettingsDlg, &GroupChatSettingsDlg::onSigCheckedGroupId, this, [&](int groupId) {
				m_stLiveContentInfo.nGroupID = groupId;
				UserConfig::instance()->setGroupId(groupId);
				m_pGroupChatSettingsDlg->close();
			});
		}
		m_pGroupChatSettingsDlg->Init(m_stLiveContentInfo.nGroupID);
	});

	connect(ui->pb_red_packet, &QPushButton::clicked, this, [&]() {
		if (m_pRedEnvelopeDlg == NULL)
		{
			m_pRedEnvelopeDlg = new RedEnvelopeDlg(this);
		}
		m_pRedEnvelopeDlg->exec();
	});

	ui->le_live_room_name->setMaxLength(20);

	ui->lb_live_room_name->setText(g_dataCentre.stUserInformation.qsRoomName);
	ui->lb_tq_number_value->setText(g_dataCentre.stUserInformation.qsTQID);
	ui->lb_room_number_value->setText(g_dataCentre.stUserInformation.qsRoomNumber);
	ui->lb_nickname->setText(g_dataCentre.stUserInformation.qsNickName);
	ui->lb_hot_value->setText(QString::number(g_dataCentre.stUserInformation.nHotValue));

	m_timerHeartbeatValidation = new QTimer(this);
	connect(m_timerHeartbeatValidation, SIGNAL(timeout()), this, SLOT(slotSubmitHeartbeatValidation()));
	m_timerHeartbeatValidation->start(HEARTBEAT_VALIDATION_INTERVAL);

	connect(CTaskNotification::GetInstance(), &CTaskNotification::signalUserPortraitDownloadFinish, this, &OBSBasic::slotUserPortraitDownloadFinished);
	connect(CTaskNotification::GetInstance(), &CTaskNotification::signalLogoutTaskNotification, this, &OBSBasic::slotLogout);
	connect(CTaskNotification::GetInstance(), &CTaskNotification::signalHeartbeatValidationTaskNotification, this, &OBSBasic::slotHeartbeatValidation);
	connect(CTaskNotification::GetInstance(), SIGNAL(signalFinishLiveTaskNotification(int, QString)), this, SLOT(slotFinishLive(int, QString)));
	//connect(CTaskNotification::GetInstance(), &CTaskNotification::signalIMAcquireGroupInfo, this, &OBSBasic::slotIMAcquireGroupInfo);
	//connect(CTaskNotification::GetInstance(), &CTaskNotification::signalAcquireLiveCategory, this, &OBSBasic::slotAcquireLiveCategory);
	connect(CTaskNotification::GetInstance(), &CTaskNotification::signalAcquireDefaultLiveContent, this, &OBSBasic::slotAcquireDefaultLiveContent);

	/*connect(CTaskNotification::GetInstance(), &CTaskNotification::signalAcquireOSSInformation, this, &OBSBasic::slotAcquireOSSInformation);
	connect(CTaskNotification::GetInstance(), &CTaskNotification::signalUploadFile, this, &OBSBasic::slotUploadFile);*/

	CController::GetInstance().PostIMLoginTask();
	//CController::GetInstance().AcquireProhibitToSpeakList();
	//CController::GetInstance().PostRepealSilent(g_dataCentre.stUserInformation.nRoomID, 22);
	//CController::GetInstance().AcquireProhibitToSpeakList();
	//CController::GetInstance().AcquireRoomAdministratorList();
	//CController::GetInstance().PostRepealRoomAdministrator(10);

	//CController::GetInstance().AcquireGroupGeneralizeList();
	//CController::GetInstance().AcquireGroupInfoTask(g_dataCentre.stUserInformation.qsGroupID);
	//CController::GetInstance().AcquireGroupInfoTask("77154438");

	//CController::GetInstance().AcquireLiveCategoryList();
	//CController::GetInstance().AcquireAppUpToDateVer();

	//CController::GetInstance().PostUploadTask("C:\\Users\\Administrator\\Desktop\\wallpaper\\wallhaven-p8gpwj.jpg");

	CController::GetInstance().AcquireDefaultLiveContent();

	m_isMLBPressed = false;
	this->m_eZoneType = CENTRE;

	// 适配光标效果
	this->setMouseTracking(true);
	this->ui->centralwidget->setMouseTracking(true);
	this->ui->centralwidget->setStyleSheet("QWidget#centralwidget{ background-color: rgba(47, 47, 47, 1); }");
	//this->ui->scenes->setStyleSheet("QListWidget {"
	...
	// 适配状态栏区域 光标无可调整大小效果的问题
	this->ui->statusbar->setMouseTracking(true);
	//this->ui->statusbar->setStyleSheet("QStatusBar { background-color: rgba(47, 47, 47, 1);}"
	//" QStatusBar QLabel { background-color: rgba(47, 47, 47, 1);}");

	this->ui->statusbar->setStyleSheet("QWidget { background-color: rgba(47, 47, 47, 255); }"); // windows7 Aero效果无法适配
	//this->ui->statusbar->setStyleSheet("QLabel { background-color: rgba(47, 47, 47, 255);}");


	QString qsBkPath(":/TQLive/images/pb_record_close.png");
	ui->recordButton->SetCustomButtonBkFileName(qsBkPath, qsBkPath, qsBkPath, qsBkPath, qsBkPath);

	QString qsNormal(":/TQLive/images/pb_live_content_normal.png");
	QString qsOver(":/TQLive/images/pb_live_content_over.png");

	

	ui->pb_live_content->SetCustomButtonBkFileName(qsNormal, qsOver, qsOver, qsOver, qsNormal);
	ui->pb_live_content->setText(QString::fromLocal8Bit("请选择分类"));

	QRect rcRegion = ui->pb_live_content->rect();
	rcRegion.setX(27);
	ui->pb_live_content->SetCustomButtonTextArea(rcRegion);
	ui->pb_live_content->SetCustomButtonTextAlignmentStyle(Qt::AlignLeft | Qt::AlignVCenter);
	//ui->pb_live_content->SetCustomButtonTextColor(QColor(255, 0, 0));

	qsNormal = ":/TQLive/images/maximum_normal.png";
	qsOver = ":/TQLive/images/maximum_over.png";
	QString qsPressed(":/TQLive/images/maximum_pressed.png");
	ui->pb_maximize->SetCustomButtonBkFileName(qsNormal, qsOver, qsOver, qsPressed, qsNormal);
	...
	QAction *renameScene = new QAction(this);
	renameScene->setShortcutContext(Qt::WidgetWithChildrenShortcut);
	connect(renameScene, SIGNAL(triggered()), this, SLOT(EditSceneName()));
	//ui->scenesDock->addAction(renameScene);

	QAction *renameSource = new QAction(this);
	...
	ui->menubar->hide();
	ui->chb_head_bar->SetBkColor(QColor(47, 47, 47, 255));
	ui->chb_head_bar->SetHostWindow(this);

	this->ui->scenes->setFocus();

	connect(ui->pb_sound_effect, SIGNAL(clicked()), this, SLOT(onShowSoundEffectMenu()));
	connect(ui->button_settings, SIGNAL(clicked()), this, SLOT(onShowSettingsIniDlg()));

	connect(ui->listWidget_material, SIGNAL(onSigAddMaterialOfCamera()), this, SLOT(onAddMaterialOfCamera()));
	connect(ui->listWidget_material, SIGNAL(onSigAddMaterialOfVideo()), this, SLOT(onAddMaterialOfVideo()));
	connect(ui->listWidget_material, SIGNAL(onSigAddMaterialOfImage()), this, SLOT(onAddMaterialOfImage()));
	connect(ui->listWidget_material, SIGNAL(onSigAddMaterialOfText()), this, SLOT(onAddMaterialOfText()));
	connect(ui->listWidget_material, SIGNAL(onSigAddMaterialOfSource()), this, SLOT(onAddMaterialOfSource()));
	connect(ui->listWidget_material, SIGNAL(onSigAddMaterialOfCapture()), this, SLOT(onAddMaterialOfCapture()));
	connect(ui->listWidget_material, SIGNAL(onSigAddMaterialOfFullScreen()), this, SLOT(onAddMaterialOfFullScreen()));
	connect(ui->listWidget_material, SIGNAL(onSigAddMaterialOfWindow()), this, SLOT(onAddMaterialOfWindow()));

	connect((QObject *)CTaskNotification::GetInstance(), SIGNAL(signalAcquireLivePushAddress(int, const QString&)), this, SLOT(slotAcquireLivePushAddress(int, const QString&)));
	connect(GlobalSignal::instance(), SIGNAL(onSigCatchException(int, QString)), this, SLOT(onCatchException(int, QString)));
	connect(GlobalSignal::instance(), SIGNAL(onSigSourceSet()), this, SLOT(on_actionSourceProperties_triggered()));
	connect(GlobalSignal::instance(), SIGNAL(onSigSourceUp()), this, SLOT(on_actionSourceUp_triggered()));
	connect(GlobalSignal::instance(), SIGNAL(onSigSourceDown()), this, SLOT(on_actionSourceDown_triggered()));
	connect(GlobalSignal::instance(), SIGNAL(onSigSourceDel()), this, SLOT(on_actionRemoveSource_triggered()));
	connect(GlobalSignal::instance(), SIGNAL(onSigReportBarrageMsgTotal(int, int, int, int)), this, SLOT(onUpdateBarrageMsgTotal(int, int, int, int)));
	connect(GlobalSignal::instance(), SIGNAL(onSigHaveSourceItems(bool)), this, SLOT(onHaveSourceItems(bool)));

	connect(HttpAgent::instance(), SIGNAL(onSigGetAppUpToDateVer(int, const QString &, const ST_VER_INFO &, const void *)),
		this, SLOT(onCheckForUpdates(int, const QString &, const ST_VER_INFO &, const void *)));
	connect(HttpAgent::instance(), SIGNAL(onSigGetChatGroupInfo(int, const QString &, const ST_GROUP_INFO &)),
		this, SLOT(onGetChatGroupInfo(int, const QString &, const ST_GROUP_INFO &)));
	connect(HttpAgent::instance(), SIGNAL(onSigIMLoginRespond()), this, SLOT(onIMLoginRespond()));
	connect(HttpAgent::instance(), SIGNAL(onSigWarningTips(int, QString, int)), this, SLOT(onWarningTips(int, QString, int)));
	connect(GlobalSignal::instance(), SIGNAL(onSigReportBarrageMsgTotal(int, int, int, int)), this, SLOT(onUpdateBarrageMsgTotal(int, int, int, int)));
	connect(CTaskNotification::GetInstance(), &CTaskNotification::signalUpdateLiveContentTaskNotification, this, &OBSBasic::slotUpdateLiveContentTask);
	connect(ui->sources, SIGNAL(onSigCreateSourceItem(OBSSceneItem)), this, SLOT(onCreateSourceItem(OBSSceneItem)));

	HttpAgent::instance()->RequestOnGetAppUpToDateVer();

	connect(m_pAudioControlWin, SIGNAL(onSigJblValueChanged(int)), this, SLOT(onJblValueChanged(int)));
	connect(m_pAudioControlWin, SIGNAL(onSigMicValueChanged(int)), this, SLOT(onMicValueChanged(int)));
	ui->horizontalLayout->insertWidget(3, m_pAudioControlWin);
	ui->stackedMixerArea->hide();

	//QTimer::singleShot(5000, this, SLOT(slotAPIVerify()));

	if (UserConfig::instance()->getIsOpenHotKey())
	{
		onHotKeyChanged(UserConfig::instance()->getMicHotKey(),
			UserConfig::instance()->getLiveHotKey(),
			UserConfig::instance()->getBarrageHotKey());
	}

	/*隐藏部分功能，二期开发*/
	ui->pb_red_packet->hide();
	//ui->preview->hide();
}
...
void OBSBasic::CreateDefaultScene(bool firstStart)
{
	...
	obs_scene_t  *scene1 = obs_scene_create(Str("Basic.Scene1"));
	obs_scene_t  *scene2 = obs_scene_create(Str("Basic.Scene2"));

	if (firstStart)
		CreateFirstRunSources();

	SetCurrentScene(scene1, true);
	obs_scene_release(scene1);
	obs_scene_release(scene2);

	disableSaving--;

	onHaveSourceItems(false);
}
...
void OBSBasic::LogScenes()
{
	...
	//m_pWaterMark = new CWaterMark(this); // 初次安装切换场景的时候需要用到

	int nSceneNum = ui->scenes->count();
	...
	for (int i = 0; i < ui->scenes->count(); i++) {
		...
		// 历史场景中如果不存在水印，添加水印
		//obs_sceneitem_t *sceneItem = obs_scene_find_source(scene, "logo_water_mark");
		//if (sceneItem == NULL)
		//{
		//	m_pWaterMark->Add(":/TQLive/water_mark.png", "image_source", scene);
		//}
	}
...
void OBSBasic::InitSystemTray()
{
	...
	if (m_pSystemTray == NULL)
	{
		...
		m_pSystemTray->setIcon(QIcon(":/TQLive/images/logo.png"));
		m_pSystemTray->setToolTip(QStringLiteral("探球直播助手"));

		QMenu *trayMenu = new QMenu(this);

		QAction *showAction = new QAction(trayMenu);
		showAction->setText(QStringLiteral("显示"));

		QAction *exitAction = new QAction(trayMenu);
		exitAction->setText(QStringLiteral("退出"));

		trayMenu->addAction(showAction);
		trayMenu->addAction(exitAction);
		m_pSystemTray->setContextMenu(trayMenu);

		connect(m_pSystemTray, SIGNAL(activated(QSystemTrayIcon::ActivationReason)),
	...
	m_pSystemTray->show();
	m_pSystemTray->showMessage(QStringLiteral("探球直播助手"), QStringLiteral("已最小化到系统托盘"));
}
...
void OBSBasic::OBSInit()
{
	...
	m_pWaterMark = new CWaterMark(this);
...
OBSBasic::~OBSBasic()
{
	SourceIniXML::instance()->m_pSourceIniList.clear();
	SourceIniJson::instance()->m_pSourceIniList.clear();
	CameraCacheJson::instance()->m_pCameraCacheList.clear();
...
bool OBSBasic::QueryRemoveSource(obs_source_t *source)
{
	...
	TipsDlg removeDlg(DIALOG_CUSTOM_OPERATOR, this);
	removeDlg.setText(text);
	removeDlg.setTitle(QTStr("ConfirmRemove.Title"));
...
void OBSBasic::SceneItemAdded(void *data, calldata_t *params)
{
	...
	obs_source_t *source = obs_sceneitem_get_source(item);
	const char *name = obs_source_get_name(source);
	if (strcmp(name, "logo_water_mark") == 0)
	{
		return;
	}
...
void OBSBasic::DrawBackdrop(float cx, float cy)
{
	...
	vec4_set(&colorVal, 0.07f, 0.07f, 0.07f, 1.0f);
...
{
	CController::GetInstance().PostIMLogoutTask();
	CController::GetInstance().PostLogoutTask();
...
void OBSBasic::slotLogout(int nStateCode, QString qsMsg)
{
	App()->quit();
	this->deleteLater();

	if (kLogout == m_eExitType)
	{
		QString qsProgramPath = QApplication::applicationFilePath();
		//QStringList slArguments = QApplication::arguments();

		QStringList slArguments;
		slArguments.append("--logout");

		QString qsWorkingDirectory = QDir::currentPath();
		QProcess::startDetached(qsProgramPath, slArguments, qsWorkingDirectory);
	}
	else if (kSessionInvalidation == m_eExitType || kNetworkConnectionDisconnected == m_eExitType)
	{
		...
		QStringList slArguments;
		slArguments.append("--invalidation");
		QString qsWorkingDirectory = QDir::currentPath();
		QProcess::startDetached(qsProgramPath, slArguments, qsWorkingDirectory);
	}

	qDebug("application logout. code:%d msg:%s", nStateCode, qsMsg.toUtf8());
}

void OBSBasic::slotHeartbeatValidation(int nStateCode, QString qsMsg)
{
	//qDebug("heartbeat validation. code:%d msg:%s", nStateCode, qsMsg.toUtf8());

	//nStateCode = 207;
	static int nValidationFailureCount = 0;

	switch (nStateCode)
	{
	case 207:
	{
		//this->slotHeartbeatValidationFailure();
		this->ValidateHeartbeatFailureProcess();

		CPromptDlg *dlg = CPromptDlg::DisplayPromptDlg(this, QString::fromLocal8Bit("登录失效，请重新登录。"));
		connect(dlg, &CPromptDlg::signalTurnoffDlg, this, &OBSBasic::slotTurnoffDlg);
		m_timerHeartbeatValidation->stop();
		m_eExitType = kSessionInvalidation;
		break;
	}
	case 3:
	case 5:
	case 401:
	{
		nValidationFailureCount++;

		if (nValidationFailureCount == 3)
		{
			QString qsPrompt;

			if (nStateCode == 401)
			{
				qsPrompt = QString::fromLocal8Bit("服务器不可达，请重新登录。");
			}
			else
			{
				qsPrompt = QString::fromLocal8Bit("网络连接断开，请重新登录。");
			}

			//CPromptDlg *dlg = CPromptDlg::DisplayPromptDlg(this, QString::fromLocal8Bit("网络连接断开，请重新登录。"));

			CPromptDlg *dlg = CPromptDlg::DisplayPromptDlg(this, qsPrompt);
			connect(dlg, &CPromptDlg::signalTurnoffDlg, this, &OBSBasic::slotTurnoffDlg);
			m_timerHeartbeatValidation->stop();
			m_eExitType = kNetworkConnectionDisconnected;
		}
...
void OBSBasic::slotIMAcquireGroupInfo(int nStatuCode, const QString &qsMsg, const ST_GROUP_INFO &stGroupInfo)
{
...
void OBSBasic::slotTurnoffDlg(int nUserData)
{
	if (nUserData == 0)
	{
		return;
...
void OBSBasic::slotSubmitHeartbeatValidation()
{
	CController::GetInstance().PostHeartbeatValidationTask(m_nBarrageMsgTotal, m_nGeneralUserTotal, m_nVipUserTotal, m_nRealBarrageMsgTotal);

	//QString qsGroupID = "CR33295736";
	//CController::GetInstance().AcquireGroupInfoTask(g_dataCentre.stUserInformation.qsGroupID);
	//CController::GetInstance().AcquireGroupMemberListTask(g_dataCentre.stUserInformation.qsGroupID);
}
...
QMenu *OBSBasic::CreateAddSourcePopupMenu()
{
	...
	addSource(popup, "capture", QT_TO_UTF8(QStringLiteral("截屏")));
...
void OBSBasic::on_actionRemoveSource_triggered()
{
	...
	if (items.size() == 1) {
		...
		if (strcmp(id, "dshow_input") == 0)
			onSourceRemoved(sourceName, m_pCameraDlgList);
		else if (strcmp(id, "ffmpeg_source") == 0)
			onSourceRemoved(sourceName, m_pMediaDlgList);
		else if (strcmp(id, "image_source") == 0) {
			onSourceRemoved(sourceName, m_pImageDlgList);
			onSourceRemoved(sourceName, m_pTextDlgList);
		}
...
void OBSBasic::on_actionSourceProperties_triggered()
{
	...
	if (source) {
		...
		const char*      id = obs_source_get_id(source);
		if (strcmp(id, "dshow_input") == 0)
			onSourcePropertiesSet(sourceName, m_pCameraDlgList);
		else if (strcmp(id, "ffmpeg_source") == 0) {
			obs_data_t *settings = obs_source_get_settings(source);
			bool            flag = obs_data_get_bool(settings, "is_local_file");
			obs_data_release(settings);
			if (!flag) {
				AddLiveSourceDlg *addLiveDlg = new AddLiveSourceDlg(this, source, false);
				addLiveDlg->Load();
				return;
			}
			onSourcePropertiesSet(sourceName, m_pMediaDlgList);
		}
		else if (strcmp(id, "image_source") == 0) {
			onSourcePropertiesSet(sourceName, m_pImageDlgList);
			onSourcePropertiesSet(sourceName, m_pTextDlgList);
		}
		else 
			CreatePropertiesWindow(source);
	}
...
void OBSBasic::on_actionSourceUp_triggered()
{
	...
	OBSScene scene = this->GetCurrentScene();
	obs_sceneitem_t *waterMarkItem = obs_scene_find_source(scene, "logo_water_mark");
	obs_sceneitem_set_order(waterMarkItem, OBS_ORDER_MOVE_TOP);
}
...
void OBSBasic::StartStreaming()
{
	...
	if (!outputHandler->StartStreaming(service)) {
		...
		ui->pb_live_content->setEnabled(true);
		ui->pb_group_settings->setEnabled(true);
	...
	if (m_nLiveAbortTpye == NORMAL_TERMINATED)
	{
		if (m_pBroadcastReminderDlg == NULL)
		{
			m_pBroadcastReminderDlg = new BroadcastReminderDlg(this);
		}

		m_pBroadcastReminderDlg->InitData();
		GlobalSignal::instance()->onSigInitStatisticsData();
	}
...
void OBSBasic::StopStreaming()
{
	...
	//if (m_nLiveAbortTpye == NORMAL_TERMINATED)
	//{
	//	m_pOffcastStatisticsDlg->InitData();
	//	GlobalSignal::instance()->onSigClearStatisticsData();
	//}
...
void OBSBasic::StreamingStart()
{
	...
	ui->streamButton->setChecked(true);
	if (UserConfig::instance()->getIsOpenHotKey()) {
		ui->streamButton->setShortcut(QKeySequence(UserConfig::instance()->getLiveHotKey()));
	}
...
void OBSBasic::StreamingStop(int code, QString last_error)
{
	...
	ui->statusbar->StreamStopped(m_nLiveAbortTpye);
	...
	// 重启直播 开始推流
	if (m_nLiveAbortTpye == LIVE_CONTENT_RESET)
	{
		CController::GetInstance().AcquireLivePushAddress(m_stLiveContentInfo.nSelectionCategoryID, m_stLiveContentInfo.nSelectionMatchID, m_stLiveContentInfo.bIsScheduleData, m_stLiveContentInfo.nScheduleRecordID, m_stLiveContentInfo.nGroupID);
	}
}
...
void OBSBasic::checkForUpdates(const ST_VER_INFO & verInfo, const QString &currVer)
{
	QString curVersion = currVer;
	QString latestVersion = verInfo.qsVersion;
	m_pIsNeedToUpdate = false;

	if (!verInfo.qsVersion.isEmpty())
	{
		int curVer = curVersion.replace(".", "").simplified().toInt();
		int latestVer = latestVersion.replace(".", "").simplified().toInt();

		if (curVer < latestVer)
		{
			if (m_pTQLiveAboutDlg != NULL)
			{
				m_pTQLiveAboutDlg->accept();
			}
			UpdateTipsDlg updateTipsDlg(this);
			updateTipsDlg.InitData(verInfo);

			m_pIsNeedToUpdate = true;
			return;
		}
	}
	...
	if (m_pTQLiveAboutDlg != NULL)
	{
		m_pTQLiveAboutDlg->InitData(true);
	}
}
...
void OBSBasic::RecordingStart()
{
	...
	QString qsBkPath(":/TQLive/images/pb_record_open.png");
	ui->recordButton->SetCustomButtonBkFileName(qsBkPath, qsBkPath, qsBkPath, qsBkPath, qsBkPath);
...
//void OBSBasic::slotHeartbeatValidationFailure()
//{
//	if (outputHandler->StreamingActive()) {
//		m_nLiveAbortTpye = EXCEPTION_TERMINATED;
//		StopStreaming();
//
//		CController::GetInstance().PostFinishLiveWorker(g_dataCentre.stUserInformation.qsRoomNumber);
//	}
//}

void OBSBasic::ValidateHeartbeatFailureProcess()
{
	if (outputHandler->StreamingActive())
	{
		m_nLiveAbortTpye = EXCEPTION_TERMINATED;
...
void OBSBasic::slotAcquireLiveCategory(int nStatusCode, const QString &qsMsg, const QList<ST_LIVE_CATEGORY_INFO> &obList, const void *userData)
{

}

void OBSBasic::onCheckForUpdates(int nStatusCode, const QString & qsMsg, const ST_VER_INFO & stVerInfo, const void * userData)
{
	if (nStatusCode == 200)
	{
		if (g_dataCentre.m_eEnvironmentType == kTestEnvironment)
		{
			checkForUpdates(stVerInfo, TEST_VERSION_NO);
		}
		else if (g_dataCentre.m_eEnvironmentType == kFormalEnvironment)
		{
			checkForUpdates(stVerInfo, VERSION_NO);
		}
	}
	else
	{
		onCatchException(nStatusCode, qsMsg);
	}
}

void OBSBasic::onCatchException(int errCode, QString errMsg)
{
	QMessageBox::warning(this, QStringLiteral("请求失败"), QStringLiteral("错误码：%1，请求失败：%2").arg(errCode).arg(errMsg));
}

void OBSBasic::onGetChatGroupInfo(int nStatusCode, const QString & qsMsg, const ST_GROUP_INFO & stGroupInfo)
{
	if (nStatusCode == 0)
	{
		if (stGroupInfo.qsGroupNumber != UserConfig::instance()->getGroupNumber())
		{
			return;
		}
		...
		QString groupName = "";
		QString groupNum = "";
		if (!stGroupInfo.qsGroupName.isEmpty())
		{
			groupName = stGroupInfo.qsGroupName;
			groupNum = QStringLiteral("(%1)人").arg(stGroupInfo.nGroupNumOfPeople);

			int length = groupName.length();
			if (length > LABEL_MAX_LENGTH)
			{
				ui->lb_group_chat_value->setToolTip(groupName);
				groupName.replace(LABEL_MAX_LENGTH, length - LABEL_MAX_LENGTH, "...");
			}
		}

		ui->lb_group_chat_value->setText(groupName);
		ui->lb_group_chat_num->setText(groupNum);
	}
}

void OBSBasic::onIMLoginRespond()
{
	HttpAgent::instance()->RequestOnGetChatGroupInfo(g_dataCentre.stUserInformation.qsGeneralizeGroupNumber);
}

void OBSBasic::ProhibitLiveProcess()
{
	if (outputHandler->StreamingActive())
	{
		m_nLiveAbortTpye = EXCEPTION_TERMINATED;
...
void OBSBasic::onWarningTips(int nStatusCode, QString sMsg, int nLimitTime)
{
	// 永久停播、警告、限时停播
	QString qsContent, qsTitle;
	int nUserData = -1;
	...
	if (nStatusCode == 3500)
	{
		...
		if (nLimitTime == 0)
		{
			qsHeadline = QString::fromLocal8Bit("直播内容违规，已被永久停播。");

			if (sMsg.isEmpty())
			{
				qsContent = qsHeadline;
			}
			else
			{
				qsContent = qsHeadline + "\n" + QString::fromLocal8Bit("停播原因：%1").arg(sMsg);
			}
		}
		else
		{
			qsHeadline = QString::fromLocal8Bit("直播内容违规，已被停播%1小时。").arg(nLimitTime);
			qsContent = qsHeadline + "\n" + QString::fromLocal8Bit("停播原因：%1").arg(sMsg);
		}

		this->ProhibitLiveProcess();
		//CPromptDlg *dlg = CPromptDlg::DisplayPromptDlg(this, QString::fromLocal8Bit("您已被停播。"));
		//CPromptDlg *dlg = CPromptDlg::DisplayPromptDlg(this, qsContent, -1, QString::fromLocal8Bit("停播提示"));
		//connect(dlg, &CPromptDlg::signalTurnoffDlg, this, &OBSBasic::slotTurnoffDlg);
		m_timerHeartbeatValidation->stop();
		qsTitle = QString::fromLocal8Bit("停播提示");
	}
	else if (nStatusCode == 3600)
	{
		qsContent = QString::fromLocal8Bit("违规内容：%1").arg(sMsg);
		qsTitle = QString::fromLocal8Bit("违规警告");
		nUserData = 0;
	}

	CPromptDlg *dlg = CPromptDlg::DisplayPromptDlg(this, qsContent, nUserData, qsTitle);
	connect(dlg, &CPromptDlg::signalTurnoffDlg, this, &OBSBasic::slotTurnoffDlg);
}

void OBSBasic::onUpdateBarrageMsgTotal(int barrageMsgTotal, int generalUserTotal, int vipUserTotal, int realBarrageMsgTotal)
{
	m_nBarrageMsgTotal = barrageMsgTotal;
	m_nGeneralUserTotal = generalUserTotal;
	m_nVipUserTotal = vipUserTotal;
	m_nRealBarrageMsgTotal = realBarrageMsgTotal;
	qDebug() << "###########BarrageMsg total: " << m_nBarrageMsgTotal;
	qDebug() << "###########Real BarrageMsg total: " << m_nRealBarrageMsgTotal;
	qDebug() << "###########GeneralUser total: " << m_nGeneralUserTotal;
	qDebug() << "###########VipUser total: " << m_nVipUserTotal;
}

void OBSBasic::onHaveSourceItems(bool bHave)
{
	if (bHave)
	{
		ui->stackedWidget->setCurrentWidget(ui->page_source);
	}
	else
	{
		ui->stackedWidget->setCurrentWidget(ui->page_tips);
	}
}

void OBSBasic::onShowSoundEffectMenu()
{
	if (m_pSoundEffectMenu != NULL)
	{
		m_pSoundEffectMenu->deleteLater();
		m_pSoundEffectMenu = NULL;
	}
	...
	auto soundEffectTypeData = UserConfig::instance()->getSoundEffectIni();
	for (auto& it : soundEffectTypeData) {
		CustomSoundEffectBtn *button = new CustomSoundEffectBtn;
		button->setData(it);
		connect(button, &CustomSoundEffectBtn::onSigPlayAudio, this, [&](QString audioFile) {
			QSound::play(audioFile);
			m_pSoundEffectMenu->close();
		});

		QListWidgetItem *itemWidget = new QListWidgetItem;
		itemWidget->setData(Qt::UserRole, it.qAduioFile);
		listWidget->addItem(itemWidget);
		listWidget->setItemWidget(itemWidget, button);
	}

	m_pSoundEffectMenu = new CustomArrowWgt(this);
	m_pSoundEffectMenu->setCenterWidget(listWidget);

	QPoint p = ui->pb_sound_effect->mapToGlobal(QPoint(0, 0));
	int posX = p.x() - (m_pSoundEffectMenu->width() - ui->pb_sound_effect->width()) / 2;
	int posY = p.y() - m_pSoundEffectMenu->height() + 9;
	m_pSoundEffectMenu->move(posX, posY);
	m_pSoundEffectMenu->show();
}

void OBSBasic::onJblValueChanged(int val)
{
	for (VolControl *vol : volumes)
		vol->setJblValue(val);
}

void OBSBasic::onMicValueChanged(int val)
{
	for (VolControl *vol : volumes)
		vol->setMicValue(val);
}

void OBSBasic::onCreateSourceItem(OBSSceneItem item)
{
	OBSSource source = obs_sceneitem_get_source(item);
	if (source) {
		QString sourceName = QT_UTF8(obs_source_get_name(source));
		const char* id = obs_source_get_id(source);
		if (strcmp(id, "dshow_input") == 0) {
			AddCameraSourceDlg *addCameraDlg = new AddCameraSourceDlg(this, source, false);
			connect(addCameraDlg, SIGNAL(onSigFlipHorizontal()), this, SLOT(on_actionFlipHorizontal_triggered()));
			m_pCameraDlgList.append(addCameraDlg);
			CController::GetInstance().WriteToLogFile("Add source of camera to sourceList: %s", QT_TO_UTF8(sourceName));
		}
		else if (strcmp(id, "ffmpeg_source") == 0) {
			bool isMedia = false;
			...
			if (isMedia) {
				AddMediaSourceDlg *addMediaDlg = new AddMediaSourceDlg(this, source, false);
				m_pMediaDlgList.append(addMediaDlg);
				CController::GetInstance().WriteToLogFile("Add source of video to sourceList: %s", QT_TO_UTF8(sourceName));
				connect(addMediaDlg, SIGNAL(onSigMediaSoundsValChanged(int, QString)),
					this, SLOT(onMediaSoundsValChanged(int, QString)));
			}
		}
		else if (strcmp(id, "image_source") == 0) {
			for (auto &iter : SourceIniJson::instance()->m_pSourceIniList) {
				if (iter.sourceName == sourceName) {
					AddTextSourceDlg *textSetDlg = new AddTextSourceDlg(this, source, false);
					m_pTextDlgList.append(textSetDlg);
					CController::GetInstance().WriteToLogFile("Add source of text to sourceList: %s", QT_TO_UTF8(sourceName));
					return;
				}
			}
			AddImageSourceDlg *imageSetDlg = new AddImageSourceDlg(this, source, false, "");
			m_pImageDlgList.append(imageSetDlg);
			CController::GetInstance().WriteToLogFile("Add source of image to sourceList: %s", QT_TO_UTF8(sourceName));
		}
	}
}
...
{
	//CController::GetInstance().PostLiveRoomSetupTask(4, "https://img.tanqiu.com/common/71d7702eabaa4f57958c1ea51d7d1a4d.jpg");

	// 判断是否选择了直播内容
	if (m_stLiveContentInfo.nSelectionCategoryID == -1)
	{
		CLiveContentSelectPromptDlg dlgPrompt;
		dlgPrompt.SetPromptDlgDisplayContent(QString::fromLocal8Bit("还未选择直播分类，是否选择分类。"));
		int result = dlgPrompt.exec();

		if (result == QDialog::Accepted)
		{
			CLiveContentSelectionDlg dlgSelect(true, this);
			//connect(&dlg, &CLiveContentSelectionDlg::signalLiveContentUpdate, (OBSBasic *)mainWindow.data(), &OBSBasic::slotUpdateLiveContent);
			connect(&dlgSelect, SIGNAL(signalLiveContentUpdate(ST_MATCH_INFO &)), this, SLOT(slotUpdateLiveContent(ST_MATCH_INFO &)));
			dlgSelect.resize(650, 430);
			dlgSelect.exec();
		...
		else   // 默认分类
		{
			...
			//if (g_dataCentre.m_eEnvironmentType == kFormalEnvironment)
			//{
			//	stMatchInfo.nMatchID = 17;
			//}
			//else
			//{
			//	stMatchInfo.nMatchID = 57;
			//}

			//stMatchInfo.bIsScheduleData = false;
			//stMatchInfo.qsCompetitionName = QString::fromLocal8Bit("综合-其它");
			//this->slotUpdateLiveContent(stMatchInfo);

			ST_MATCH_INFO stMatchInfo;
			stMatchInfo.nCategoryID = m_stDefaultLiveContent.nCategoryID;
			stMatchInfo.nMatchID = m_stDefaultLiveContent.nClassifyID;
			stMatchInfo.bIsScheduleData = false;
			stMatchInfo.qsCompetitionName = QString("%1-%2").arg(m_stDefaultLiveContent.qsCategoryName).arg(m_stDefaultLiveContent.qsClassifyName);
			this->slotUpdateLiveContent(stMatchInfo);

		}
		//connect(dlg, &CLiveContentSelectPromptDlg::signalTurnoffDlg, this, &OBSBasic::CLiveContentSelectPromptDlg);
	}
	...
	if (outputHandler->StreamingActive()) {
		...
		CController::GetInstance().WriteToLogFile("quit live start.");

		m_nLiveRecordID = g_dataCentre.stUserInformation.m_nLiveRecordID;
		CController::GetInstance().PostFinishLiveWorker(g_dataCentre.stUserInformation.qsRoomNumber);

		m_nLiveAbortTpye = NORMAL_TERMINATED;
		StopStreaming();

		// 退出正在直播状态 启用直播内容&&推广群更改
		ui->pb_live_content->setEnabled(true);
		ui->pb_group_settings->setEnabled(true);
	}
	else
	{
		if (m_pIsNeedToUpdate)
		{
			QMessageBox::warning(this, QStringLiteral("开播异常"), QStringLiteral("当前版本低，请升级更新您的客户端后再来尝试开播！！！"));
			ui->streamButton->setEnabled(true);
			return;
		}
		...
		// 正在直播状态 禁用直播内容&&推广群更改
		//ui->pb_live_content->setEnabled(false);
		ui->pb_group_settings->setEnabled(false);

		CController::GetInstance().WriteToLogFile("acquire live push address start. categoryID:%d matchID:%d scheduleData:%d", m_stLiveContentInfo.nSelectionCategoryID, m_stLiveContentInfo.nSelectionMatchID, m_stLiveContentInfo.bIsScheduleData);

		int nCategoryID = ui->pb_live_content->GetUserData();
		CController::GetInstance().AcquireLivePushAddress(m_stLiveContentInfo.nSelectionCategoryID, m_stLiveContentInfo.nSelectionMatchID,
			m_stLiveContentInfo.bIsScheduleData, m_stLiveContentInfo.nScheduleRecordID, m_stLiveContentInfo.nGroupID);

		//static int nCount = 0;
		//nCount++;
		//qDebug("*******start live. count:%d", nCount);
...
{
	config_t *config = basicConfig;
	return basicConfig;
}

obs_source_t * OBSBasic::GetAudioSource() const
{
	obs_source_t *source = nullptr;
	for (VolControl *vol : volumes) {
		if (vol->isAudioVol()) {
...
static void GetItemBox(obs_sceneitem_t *item, vec3 &ptLeftTop, vec3 &br)
{
	matrix4 boxTransform;
	obs_sceneitem_get_box_transform(item, &boxTransform);

	vec3_set(&ptLeftTop, M_INFINITE, M_INFINITE, 0.0f);
	...
	auto GetMinPos = [&](float x, float y)
	{
		...
		vec3_min(&ptLeftTop, &ptLeftTop, &pos);
		vec3_max(&br, &br, &pos);
	};
...
{
	vec3 ptLeftTop, br;
	GetItemBox(item, ptLeftTop, br);
	return ptLeftTop;
}

static void SetItemTL(obs_sceneitem_t *item, const vec3 &ptLeftTop)
{
	vec3 newTL;
	vec2 pos;
	...
	pos.x += ptLeftTop.x - newTL.x;
	pos.y += ptLeftTop.y - newTL.y;
...
{
	vec3 ptLeftTop, br, itemCenter, screenCenter, offset;
	obs_video_info ovi;
	...
	vec3_sub(&itemCenter, &br, &ptLeftTop);
	vec3_mulf(&itemCenter, &itemCenter, 0.5f);
	vec3_add(&itemCenter, &itemCenter, &ptLeftTop);

	vec3_sub(&offset, &screenCenter, &itemCenter);
	vec3_add(&ptLeftTop, &ptLeftTop, &offset);
...
void OBSBasic::Nudge(int dist, MoveDir m_eZoneType)
{
...
void OBSBasic::UpdateTitleBar()
{
	...
	setWindowTitle(QStringLiteral("探球直播助手"));
}
...
// 适配设置FramelessWindowHint属性 界面不刷新的问题 显示与隐藏来源效果无法切换
void OBSBasic::showEvent(QShowEvent *ev)
{
...
//void OBSBasic::InitializeCusorDropArea()
//{
//	QRect rect = this->rect();
//	QPoint ptTopLeft = rect.topLeft();
//	QPoint ptRightLeft = rect.bottomRight();
//
//	m_szDropArea[LEFTTOP].setRect(ptTopLeft.x(), ptTopLeft.y(), ZONE_MEASURE, ZONE_MEASURE);
//	m_szDropArea[TOP].setRect(ptTopLeft.x() + ZONE_MEASURE, ptTopLeft.y(), rect.width()ZONE_MEASURE, ZONE_MEASURE);
//}


void OBSBasic::SetCursorShape(const QPoint &ptCursorPos)
{
	QRect rect = this->rect();
	QPoint ptLeftTop = mapToGlobal(rect.topLeft());
	QPoint ptRightBottom = mapToGlobal(rect.bottomRight());
	...
	//if (x >= ptLeftTop.x() + ZONE_MEASURE && x <= ptRightBottom.x() - ZONE_MEASURE && y >= ptLeftTop.y() + ZONE_MEASURE && y <= ptRightBottom.y() - ZONE_MEASURE)
	//{
	//	// 默认
	//	m_eZoneType = NONE;
	//	this->setCursor(QCursor(Qt::ArrowCursor));
	//}
	//else if (ptLeftTop.x() + ZONE_MEASURE >= x && ptLeftTop.x() <= x && ptLeftTop.y() + ZONE_MEASURE >= y && ptLeftTop.y() <= y)
	//{
	//	// 左上角
	//	m_eZoneType = LEFTTOP;
	//	this->setCursor(QCursor(Qt::SizeFDiagCursor));  // 设置鼠标形状
	//}
	//else if (x >= ptRightBottom.x() - ZONE_MEASURE && x <= ptRightBottom.x() && y >= ptRightBottom.y() - ZONE_MEASURE && y <= ptRightBottom.y()) 
	//{
	//	// 右下角
	//	m_eZoneType = RIGHTBOTTOM;
	//	this->setCursor(QCursor(Qt::SizeFDiagCursor));
	//}
	//else if (x <= ptLeftTop.x() + ZONE_MEASURE && x >= ptLeftTop.x() && y >= ptRightBottom.y() - ZONE_MEASURE && y <= ptRightBottom.y()) 
	//{
	//	//左下角
	//	m_eZoneType = LEFTBOTTOM;
	//	this->setCursor(QCursor(Qt::SizeBDiagCursor));
	//}
	//else if (x <= ptRightBottom.x() && x >= ptRightBottom.x() - ZONE_MEASURE && y >= ptLeftTop.y() && y <= ptLeftTop.y() + ZONE_MEASURE) \
	//{
	//// 右上角
	//m_eZoneType = RIGHTTOP;
	//this->setCursor(QCursor(Qt::SizeBDiagCursor));
	//}
	//else if (x <= ptLeftTop.x() + ZONE_MEASURE && x >= ptLeftTop.x()) {
	//	// 左边
	//	m_eZoneType = LEFT;
	//	this->setCursor(QCursor(Qt::SizeHorCursor));
	//}
	//else if (x <= ptRightBottom.x() && x >= ptRightBottom.x() - ZONE_MEASURE) {
	//	// 右边
	//	m_eZoneType = RIGHT;
	//	this->setCursor(QCursor(Qt::SizeHorCursor));
	//}
	//else if (y >= ptLeftTop.y() && y <= ptLeftTop.y() + ZONE_MEASURE) {
	//	// 上边
	//	m_eZoneType = TOP;
	//	this->setCursor(QCursor(Qt::SizeVerCursor));
	//}
	//else if (y <= ptRightBottom.y() && y >= ptRightBottom.y() - ZONE_MEASURE) {
	//	// 下边
	//	m_eZoneType = BOTTOM;
	//	this->setCursor(QCursor(Qt::SizeVerCursor));
	//}
	...
	for (int i = 0; i < 9; i++)
	{
		...
		{
			m_eZoneType = (EN_ZONE_TYPE)i;
			break;
		}

		if (i == 8)
		{
			qDebug("hit test no zone.");
		}
	}

	//qDebug("cursor current area. position:%d", m_eZoneType);
...
//void OBSBasic::enterEvent(QEvent *event)
//{
//	//this->setCursor(QCursor(Qt::ArrowCursor));
//	//QWidget::enterEvent(event);
//	QEnterEvent *enterEvent = dynamic_cast<QEnterEvent *>(event);
//	QPoint ptCursorPos = enterEvent->globalPos();
//	this->SetCursorShape(ptCursorPos);
//}
...
void OBSBasic::leaveEvent(QEvent *event)
{
	this->setCursor(QCursor(Qt::ArrowCursor));
	m_eZoneType = CENTRE;						// 重置状态为CENTRE很重要 导致问题II 进入别的区域并不会导致离开

	QMainWindow::leaveEvent(event);
}

#define nMinimumWidth 1060
#define nMinimumHeight 690

void OBSBasic::mouseMoveEvent(QMouseEvent *event)
{
	// 左键按下的时候能接收到位置为负的鼠标事件
	//qDebug("mouse move event. x:%d y:%d", event->pos().x(), event->pos().y());							
	//QPoint ptCursorPos = event->pos();						
	//QRect rcWindowArea = this->rect();
	//QPoint ptLeftTop = rcWindowArea.topLeft();
	//QPoint ptRightBottom = rcWindowArea.bottomRight();

	// scenario one
	QPoint ptCursorPos = event->globalPos();				// 没有鼠标位置为负的情况

	QRect rcWindowArea = this->rect();
	QPoint ptLeftTop = mapToGlobal(rcWindowArea.topLeft());
	QPoint ptRightBottom = mapToGlobal(rcWindowArea.bottomRight());
	...
	{
		this->SetCursorShape(ptCursorPos);
		//QMainWindow::mouseMoveEvent(event);					// 采用父类的默认处理，继续传递 此处无需继续传递
	}
	else
	{
		if (m_eZoneType != CENTRE)
		{
			...
			//qDebug("*******zoneType:%d cursorX:%d rBottomX:%d srcWidth:%d dstWidth:%d", m_eZoneType, ptCursorPos.x(), rcDestArea.bottomRight().x(), rcDestArea.width(), ptCursorPos.x() - ptLeftTop.x());
			
			switch (m_eZoneType) 
			{
			case LEFT:
			{
				// 左边距缩放区域横向缩放，缩小时，不能小于最小宽度,否则无需处理
				//if (ptRightBottom.x() - ptCursorPos.x() < nMinimumWidth) 
				//{
				...
				if (ptRightBottom.x() - ptCursorPos.x() >= nMinimumWidth)
				{
					rcDestArea.setLeft(ptCursorPos.x());					// 放大缩小同时处理
				}
			...
			case RIGHT:
			{
				// 右边距缩放区域横向缩放，缩小时，不能小于最小宽度，否则无需处理
				// rcDestArea.setWidth(ptCursorPos.x() - ptLeftTop.x());

				if (ptCursorPos.x() - ptLeftTop.x() >= nMinimumWidth)
				{
					rcDestArea.setRight(ptCursorPos.x());					// 放大缩小同时处理
				}
			...
			case TOP:
			{
				// 上边距缩放区域纵向缩放，缩小时，不能小于最小高度，否则无需处理
				if (ptRightBottom.y() - ptCursorPos.y() >= nMinimumHeight) 
				{
					rcDestArea.setTop(ptCursorPos.y());
				}
			...
			case BOTTOM:
			{
				if (ptCursorPos.y() - ptLeftTop.y() >= nMinimumHeight)
				{
					rcDestArea.setBottom(ptCursorPos.y());
				}
			...
			case LEFTTOP:
			{
				// 上左边距缩放区域横向纵向缩放，缩小时，不能小于最小宽度；不能小于最小高度
				if (ptRightBottom.x() - ptCursorPos.x() >= nMinimumWidth)
				{
					rcDestArea.setLeft(ptCursorPos.x());
				}
				if (ptRightBottom.y() - ptCursorPos.y() >= nMinimumHeight)
				{
					rcDestArea.setTop(ptLeftTop.y());
				}
			...
			case RIGHTTOP:
			{
				if (ptCursorPos.x() - ptLeftTop.x() >= nMinimumWidth)
				{
					rcDestArea.setRight(ptCursorPos.x());					// 横向放大缩小同时处理
				}

				if (ptRightBottom.y() - ptCursorPos.y() >= nMinimumHeight)
				{
					rcDestArea.setTop(ptLeftTop.y());						// 纵向放大缩小同时处理
				}
			...
			case LEFTBOTTOM:
			{
				if (ptRightBottom.x() - ptCursorPos.x() >= nMinimumWidth)
				{
					rcDestArea.setLeft(ptCursorPos.x());					// 横向放大缩小同时处理
				}
			...
			this->setGeometry(rcDestArea);
			//this->resize(rcDestArea.width(), rcDestArea.height());
...
//}
//
//void OBSBasic::mousePressEvent(QMouseEvent *e)
//{
//#ifdef Q_OS_WIN
...
//}
//void OBSBasic::mouseMoveEvent(QMouseEvent *e)
//{
//#ifdef Q_OS_WIN
...
void OBSBasic::slotShowMaximizedOrNormal()
{
	if (m_eDisplayState == kMaximizedState)
	{
		//ui->pb_maximize->setDown(false);
		this->showNormal();
		ui->pb_maximize->SetBtnState(NORMAL_STATE);
		m_eDisplayState = kNormalState;
	...
	else
	{
		...
		//int nDCRWidth = pDesktop->availableGeometry().width();
		//int nDCRHeight = pDesktop->availableGeometry().height();
		
		//this->resize(nDCRWidth, nDCRHeight);
		//this->setGeometry(0, 0, nDCRWidth, nDCRHeight);

		//ui->pb_maximize->setDown(true);
		this->showMaximized();
		ui->pb_maximize->SetBtnState(DOWN_STATE);
		m_eDisplayState = kMaximizedState;
	}
}

void OBSBasic::resizeEvent(QResizeEvent *event)
{
	qDebug("width:%d height:%d", event->size().width(), event->size().height());
	//m_rcWndMeasure = this->frameGeometry();
}

void OBSBasic::slotModifyLiveRoomName()
{
	ui->sw_live_room_name_management->setCurrentIndex(1);
	QString qsLiveRoomName = ui->lb_live_room_name->text();
	ui->le_live_room_name->setText(qsLiveRoomName);
	ui->le_live_room_name->setFocus();
}

void OBSBasic::slotAcquireLivePushAddress(int nStateCode, const QString& qsMsg)
{
	CController::GetInstance().WriteToLogFile("acquire live push address end. errorCode:%d errorDesc:%s", nStateCode, qsMsg.toLocal8Bit().constData());
	...
	else
	{
		...
		if (211 == nStateCode)
		{
			CUniversalPromptDlg dlgPrompt;
			dlgPrompt.SetPromptDlgDisplayContent(QString::fromLocal8Bit("服务器繁忙，请稍后再试。"));
			int result = dlgPrompt.exec();
		}
	}
}

void OBSBasic::slotFinishLive(int nStatusCode, QString qsMsg)
{
	CController::GetInstance().WriteToLogFile("quit live end. errorCode:%d errorDesc:%s", nStatusCode, qsMsg.toLocal8Bit().constData());

	// 重置直播记录ID
	m_nLiveRecordID = g_dataCentre.stUserInformation.m_nLiveRecordID;
	g_dataCentre.stUserInformation.m_nLiveRecordID = 0;
...
void OBSBasic::slotModificationCancel()
{
	ui->sw_live_room_name_management->setCurrentIndex(0);
}

void OBSBasic::slotModificationSave()
{

	QString qsLiveRoomName = ui->le_live_room_name->text().trimmed();
	int nLen = qsLiveRoomName.length();


	if (nLen < 5)
	{
		ui->le_live_room_name->clear();
		ui->le_live_room_name->setFocus();
		...
		QToolTip::showText(ui->le_live_room_name->mapToGlobal(ui->le_live_room_name->pos()), QStringLiteral("请输入5~20字的房间名称！"));
	}
	else
	{
		ui->sw_live_room_name_management->setCurrentIndex(0);

		//bool result = ui->le_live_room_name->isModified();

		//if (result == true)
		//{
		//	CController::GetInstance().PostLiveRoomSetupTask(2, qsLiveRoomName);
		//	ui->lb_live_room_name->setText(qsLiveRoomName);
		//	g_dataCentre.stUserInformation.qsRoomName = qsLiveRoomName;
		//}

		if (qsLiveRoomName.compare(ui->lb_live_room_name->text()) != 0)
		{
			CController::GetInstance().PostLiveRoomSetupTask(2, qsLiveRoomName);
			ui->lb_live_room_name->setText(qsLiveRoomName);
			g_dataCentre.stUserInformation.qsRoomName = qsLiveRoomName;
...
void OBSBasic::slotUserPortraitDownloadFinished()
{
	//ui->lb_user_portrait->setPixmap(g_dataCentre.stUserInformation.qsPortrait);
	ui->lb_user_portrait->SetBkImagePath(g_dataCentre.stUserInformation.qsPortrait);
	ui->lb_user_portrait->repaint();
}

void OBSBasic::onLiveRoomTitleChanged()
{
	ui->lb_live_room_name->setText(g_dataCentre.stUserInformation.qsRoomName);
}

void OBSBasic::onHotKeyChanged(QString micHotKey, QString liveHotKey, QString barrageHotKey)
{
	m_pAudioControlWin->SetMicHotKey(micHotKey);
	ui->streamButton->setShortcut(QKeySequence(liveHotKey));
	ui->pb_bullet_screen->setShortcut(QKeySequence(barrageHotKey));
}

void OBSBasic::onAddMaterialOfCamera()
{
	AddCameraSourceDlg *cameraSetDlg = new AddCameraSourceDlg(this, NULL, true);
	connect(cameraSetDlg, SIGNAL(onSigFlipHorizontal()), this, SLOT(on_actionFlipHorizontal_triggered()));
	m_pCameraDlgList.append(cameraSetDlg);
	CController::GetInstance().WriteToLogFile("Init camera to sourceList: %s", QT_TO_UTF8(cameraSetDlg->getSourceName()));

	m_pWaterMark->BringLayerToFront();
	cameraSetDlg->Load();
}

void OBSBasic::onAddMaterialOfVideo()
{
	AddMediaSourceDlg *addMediaDlg = new AddMediaSourceDlg(this, NULL, true);
	m_pMediaDlgList.append(addMediaDlg);
	connect(addMediaDlg, SIGNAL(onSigMediaSoundsValChanged(int, QString)),
		this, SLOT(onMediaSoundsValChanged(int, QString)));
	CController::GetInstance().WriteToLogFile("Init video to sourceList: %s", QT_TO_UTF8(addMediaDlg->getSourceName()));

	m_pWaterMark->BringLayerToFront();
	addMediaDlg->Load();
}

void OBSBasic::onAddMaterialOfImage()
{
	QString path = QFileDialog::getOpenFileName(NULL, QStringLiteral("请选择要打开的图片文件"), "");
	if (path.isEmpty())
		return;

	AddImageSourceDlg *addImageDlg = new AddImageSourceDlg(this, NULL, true, path);
	m_pImageDlgList.append(addImageDlg);
	m_pWaterMark->BringLayerToFront();
	addImageDlg->Load();
}

void OBSBasic::onAddMaterialOfText()
{
	//AddSource("text_gdiplus");
	AddTextSourceDlg *addTextDlg = new AddTextSourceDlg(this, NULL, true);
	m_pTextDlgList.append(addTextDlg);
	m_pWaterMark->BringLayerToFront();
	addTextDlg->Load();
}

void OBSBasic::onAddMaterialOfSource()
{
	AddLiveSourceDlg *addLiveDlg = new AddLiveSourceDlg(this, NULL, true);
	m_pWaterMark->BringLayerToFront();
	addLiveDlg->Load();
}

void OBSBasic::onAddMaterialOfCapture()
{
	AddCaptureSourceDlg *addCaptureDlg = new AddCaptureSourceDlg(this);
	m_pWaterMark->BringLayerToFront();
	addCaptureDlg->Load();
}

void OBSBasic::onAddMaterialOfFullScreen()
{
	AddFullScreenSourceDlg *addFullScreenDlg = new AddFullScreenSourceDlg(this);
	m_pWaterMark->BringLayerToFront();
	addFullScreenDlg->Load();
}

void OBSBasic::onAddMaterialOfWindow()
{
	AddSource("window_capture");
}

void OBSBasic::onMediaSoundsValChanged(int val, QString sourceName)
{
	for (VolControl *vol : volumes) {
		if (sourceName == vol->GetName()) {
			vol->setJblValue(val);
			break;
		}
	}
...
void OBSBasic::onShowAboutDlg()
{
	m_pSystemSettingsDlg->close();

	if (m_pTQLiveAboutDlg == NULL)
	{
		m_pTQLiveAboutDlg = new TQLiveAboutDlg(this);
	}
	m_pTQLiveAboutDlg->InitData(false);
}

void OBSBasic::onShowSettingsIniDlg()
{
	if (m_pSystemSettingsDlg != NULL)
		m_pSystemSettingsDlg->close();

	SettingsIniDlg dlg(this);
	connect(&dlg, SIGNAL(onLiveRoomTitleChanged()), this, SLOT(onLiveRoomTitleChanged()));
	connect(&dlg, SIGNAL(onHotKeyChanged(QString, QString, QString)), this,
		SLOT(onHotKeyChanged(QString, QString, QString)));
	connect(&dlg, SIGNAL(onWaterMarkStateChanged(int)), this, SLOT(onWaterMarkStateChanged(int)));
	dlg.InitPage();
}

void OBSBasic::onShowLogoutDlg()
{
	m_pSystemSettingsDlg->close();

	TipsDlg *tipsDlg = new TipsDlg(DIALOG_LOGOUT_ACCOUNT_OPERATOR, this);
...
void OBSBasic::onShowFeedbackDlg()
{
	m_pSystemSettingsDlg->close();

	FeedbackDlg dlg(this);
	dlg.exec();
}

void OBSBasic::onExitApplication()
{
	if (m_pExitAppTipsDlg == NULL)
	{
		m_pExitAppTipsDlg = new ExitAppTipsDlg(this);
		connect(m_pExitAppTipsDlg, SIGNAL(onSigConfirmClicked()), this, SLOT(onHandlerExitApp()));
	}

	if (UserConfig::instance()->getExitTipsFlag())
	{
		m_pExitAppTipsDlg->InitPage();
		m_pExitAppTipsDlg->exec();
	}
	else
	{
		onHandlerExitApp();
...
void OBSBasic::onHandlerExitApp()
{
...
void OBSBasic::activated(QSystemTrayIcon::ActivationReason reason)
{
...
void OBSBasic::ResetCurrentService()
{
	...
	//QString qsServerName;
	//m_qsRTMPUrl = "rtmp://push.tanqiu.com/tantan/";
	//m_qsKey = "tantanS?auth_key=1566571052-0-0-0bc530463fc56b55438cf0110927efd0";

	QString qsLivePushAddress = g_dataCentre.stUserInformation.qsLivePushAddress;

	int pos = qsLivePushAddress.lastIndexOf("/");
	if (pos != -1)
	{
		m_qsRTMPUrl = qsLivePushAddress.left(pos + 1);
		m_qsKey = qsLivePushAddress.mid(pos + 1);

	}

	obs_data_set_string(settings, "server", QT_TO_UTF8(m_qsRTMPUrl));
	obs_data_set_bool(settings, "use_auth", false);
	//obs_data_set_bool(settings, "bwtest", ui->bandwidthTestEnable->isChecked());
	obs_data_set_string(settings, "key", QT_TO_UTF8(m_qsKey));
...
void OBSBasic::CancelCurrentSource(QString sourceName)
{
	vector<OBSSceneItem> items;
	obs_scene_enum_items(GetCurrentScene(), remove_items, &items);
...
//void OBSBasic::slotUpdateLiveContent(const QString &qsCategoryName, const QString &qsContentName)
//{
//	QString qsDisplay = qsCategoryName + " " + qsContentName;
//	ui->pb_live_content->setText(qsDisplay);
//}

// 直播内容更新
void OBSBasic::slotUpdateLiveContent(ST_MATCH_INFO &stMatchInfo)
{
	// solution II 直接修改无需重启直播
	if (outputHandler->StreamingActive())					// 直播状态需立即同步更新服务端直播内容
	{
		CController::GetInstance().WriteToLogFile("update the live content start. categoryID:%d matchID:%d", stMatchInfo.nCategoryID, stMatchInfo.nMatchID);
		CController::GetInstance().PostUpdateLiveContentTask(stMatchInfo.nCategoryID, stMatchInfo.nMatchID, NULL);
	}

	QString qsDisplayContent;

	if (stMatchInfo.nCategoryID != SYNTHESIS_TYPE)
	{
		stMatchInfo.qsContent = stMatchInfo.qsHostTeam + "vs" + stMatchInfo.qsGuestTeam;
		qsDisplayContent = stMatchInfo.qsCompetitionName + " " + stMatchInfo.qsContent;
	}
	else
	{
		qsDisplayContent = stMatchInfo.qsCompetitionName;
	}

	ui->pb_live_content->setText(qsDisplayContent);
	ui->pb_live_content->SetUserData(stMatchInfo.nCategoryID);

	m_stLiveContentInfo.nSelectionCategoryID = stMatchInfo.nCategoryID;
	m_stLiveContentInfo.nSelectionMatchID = stMatchInfo.nMatchID;
	m_stLiveContentInfo.bIsScheduleData = stMatchInfo.bIsScheduleData;

	if (stMatchInfo.bIsScheduleData == true)
	{
		m_stLiveContentInfo.nScheduleRecordID = stMatchInfo.nScheduleRecordID;
	}

	// solution I 正在直播状态 重启直播
	//bool isActive = outputHandler->StreamingActive();
	//if (isActive == true)
	//{
	//	CController::GetInstance().WriteToLogFile("restart the live.");
	//	// 结束推流
	//	ui->streamButton->setEnabled(false);
	//	m_nLiveAbortTpye = LIVE_CONTENT_RESET;
	//	CController::GetInstance().PostFinishLiveWorker(g_dataCentre.stUserInformation.qsRoomNumber);
	//	StopStreaming();							// 结束推流通知以后才能重新启动推流
	//	//ui->pb_group_settings->setEnabled(true);
	...
	//if (outputHandler->StreamingActive())
	//{
	//	ST_MATCH_INFO *pMatchInfo = new ST_MATCH_INFO();
	//	*pMatchInfo = stMatchInfo;

	//	CController::GetInstance().WriteToLogFile("update the live content start.");
	//	CController::GetInstance().PostUpdateLiveContentTask(stMatchInfo.nCategoryID, stMatchInfo.nMatchID, pMatchInfo);
	//}
}

void OBSBasic::slotModifyLiveContent()
{
	// 直播内容选择页面
	CLiveContentSelectionDlg dlg(true, this);
	//connect(&dlg, &CLiveContentSelectionDlg::signalLiveContentUpdate, this, &OBSBasic::slotUpdateLiveContent);
	connect(&dlg, SIGNAL(signalLiveContentUpdate(ST_MATCH_INFO &)), this, SLOT(slotUpdateLiveContent(ST_MATCH_INFO &)));
	dlg.resize(650, 430);
	dlg.exec();
}

void OBSBasic::slotUpdateLiveContentTask(int nStatusCode, QString qsMsg, const void *userData)
{
	CController::GetInstance().WriteToLogFile("update the live content end. errorCode:%d errorDesc:%s", nStatusCode, qsMsg.toLocal8Bit().constData());

	//ST_MATCH_INFO *pMatchInfo = (ST_MATCH_INFO *)userData;

	//if (nStatusCode == 200)
	//{
	//	QString qsDisplay;

	//	if (pMatchInfo->nCategoryID != SYNTHESIS_TYPE)
	//	{
	//		pMatchInfo->qsContent = pMatchInfo->qsHostTeam + "vs" + pMatchInfo->qsGuestTeam;
	//		qsDisplay = pMatchInfo->qsCompetitionName + " " + pMatchInfo->qsContent;
	//	}
	//	else
	//	{
	//		qsDisplay = pMatchInfo->qsCompetitionName;
	//	}

	//	ui->pb_live_content->setText(qsDisplay);
	//	ui->pb_live_content->SetUserData(pMatchInfo->nCategoryID);

	//	m_stLiveContentInfo.nSelectionCategoryID = pMatchInfo->nCategoryID;
	//	m_stLiveContentInfo.nSelectionMatchID = pMatchInfo->nMatchID;
	//	m_stLiveContentInfo.bIsScheduleData = pMatchInfo->bIsScheduleData;

	//	if (pMatchInfo->bIsScheduleData == true)
	//	{
	//		m_stLiveContentInfo.nScheduleRecordID = pMatchInfo->nScheduleRecordID;
	//	}
	//}
...
void OBSBasic::slotAcquireDefaultLiveContent(int nStatuCode, const QString &qsMsg, const ST_LIVE_CONTENT &stLiveContent)
{
	if (200 == nStatuCode)
	{
		m_stDefaultLiveContent = stLiveContent;
	}

}

void OBSBasic::slotAPIVerify()
{
	///* 初始化OSS账号信息 */
	...
	//{
	//		/* 异常处理 */
	//	qDebug() << "PutObject fail" << ",code:" << QString::fromStdString(outcome.error().Code()) 
...
void OBSBasic::onWaterMarkStateChanged(int state)
{
	//if (state == 0)
	//{
	//	m_pWaterMark->ResetBackgroudLayer(NULL);
	//}
	//else
	//{
	//	m_pWaterMark->CompositeLayer();
	//}
	...
	if (state == 0)
	{
		m_pWaterMark->ShowWithNoRoomNO(source);
	}
	else
	{
		m_pWaterMark->ShowWithRoomNO(source);
	}
}

void OBSBasic::slotAcquireOSSInformation(int nStatusCode, const QString &qsMsg, const ST_OSS_INFORMATION &stOSSInformation, const void *userData)
{
	//QString qsFilePath("C:\\Users\\Administrator\\Desktop\\wallpaper\\wallhaven-p8mk6j.jpg");
	QString qsFilePath("C:\\Users\\Administrator\\Desktop\\wallpaper\\navicat121_sqlite_en_x64.exe");
	CController::GetInstance().UploadFileTask(stOSSInformation, qsFilePath);
}

void OBSBasic::slotUploadFile(int nStatusCode, const QString &qsMsg, const QString &qsFileID, const void *userData)
{
...
{
	if (watched == ui->preview || watched == ui->wd_display_user_info || watched == ui->wd_control_panel || watched == ui->sourcesFrame
		|| watched == ui->materialFrame || watched == ui->scenes)
	{
