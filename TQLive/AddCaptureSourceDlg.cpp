...
AddCaptureSourceDlg::AddCaptureSourceDlg(QWidget *parent)
	: QDialog(parent)
...
AddCaptureSourceDlg::~AddCaptureSourceDlg()
{
	
}

void AddCaptureSourceDlg::closeEvent(QCloseEvent * event)
{
	Q_UNUSED(event)
...
QString AddCaptureSourceDlg::getSourceName()
{
	QString sourceName;
	if (m_pSource) {
...
void AddCaptureSourceDlg::StartToCapture()
{
	if (m_pSource && monitor_property) {
		const char       *name = obs_property_name(monitor_property);
		obs_combo_format  format = obs_property_list_format(monitor_property);
...
void AddCaptureSourceDlg::PorpertiesUpdate(int left, int right, int top, int bottom)
{
	obs_source_t *existing_filter;
	string name = obs_source_get_display_name("crop_filter");
	if (!name.empty() && m_pSource) {
		...
		obs_source_t *filter = obs_source_create("crop_filter", "crop_my", nullptr, nullptr);
		if (filter) {
			obs_source_filter_add(m_pSource, filter);

			obs_data_t *settings = obs_source_get_settings(filter);
			obs_data_set_int(settings, "left", left);
			obs_data_set_int(settings, "right", right);
			obs_data_set_int(settings, "top", top);
			obs_data_set_int(settings, "bottom", bottom);
...
void AddCaptureSourceDlg::onConfirmClicked()
{
	main->SaveProject();
	hide();
...
void AddCaptureSourceDlg::onSelectedChanged(int index)
{
	QVariant data;
...
void AddCaptureSourceDlg::InitUI()
{
...
void AddCaptureSourceDlg::InitConnect()
{
...
void AddCaptureSourceDlg::InitSource()
{
	...
	if (!m_pSource) {
		...
		while (true)
		{
			QString name = QStringLiteral("屏幕截图%1").arg(index);
...
void AddCaptureSourceDlg::onDialogClosed()
{
