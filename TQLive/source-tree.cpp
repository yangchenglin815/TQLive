...
void SourceTreeItem::enterEvent(QEvent * event)
{
	...
	if (set) {
		set->setHidden(bHidden);
	}

	up->setHidden(bHidden);
	down->setHidden(bHidden);
	del->setHidden(bHidden);
}
	  ...
	  sceneitem    (sceneitem_)
{
	...
	label->setStyleSheet(QString("font-family:%1;font-size:12px;color:#DCDCDC;").arg(QStringLiteral("Î¢ÈíÑÅºÚ")));

	up = new VisibilityPushButton(SOURCEITEM_UP);
	connect(up, &QPushButton::clicked, this, [&]() {
		emit onSigSourceHandler(sceneitem, SOURCEITEM_UP);
	});
	up->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Maximum);
	up->setMaximumSize(10, 6);

	down = new VisibilityPushButton(SOURCEITEM_DOWN);
	connect(down, &QPushButton::clicked, this, [&]() {
		emit onSigSourceHandler(sceneitem, SOURCEITEM_DOWN);
	});
	down->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Maximum);
	down->setMaximumSize(10, 6);

	del = new VisibilityPushButton(SOURCEITEM_DEL);
	connect(del, &QPushButton::clicked, this, [&]() {
		emit onSigSourceHandler(sceneitem, SOURCEITEM_DEL);
	});
	del->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Maximum);
	del->setMaximumSize(10, 12);
	...
	boxLayout->setContentsMargins(12, 1, 10, 1);
	...
	if (strcmp(id, "monitor_capture") != 0) {
		set = new VisibilityPushButton(SOURCEITEM_SETTINGS);
		connect(set, &QPushButton::clicked, this, [&]() {
			emit onSigSourceHandler(sceneitem, SOURCEITEM_SETTINGS);
		});
		set->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Maximum);
		set->setMaximumSize(10, 10);
		boxLayout->addWidget(set);
		set->setHidden(true);
	}
...
void SourceTreeItem::ExitEditMode(bool save)
{
	...
	boxLayout->insertWidget(3, label);
...
void SourceTreeModel::SceneChanged()
{
	...
	GlobalSignal::instance()->onSigHaveSourceItems(bHave);
...
void SourceTreeModel::Add(obs_sceneitem_t *item)
{
	if (obs_sceneitem_is_group(item)) {
		...
		GlobalSignal::instance()->onSigHaveSourceItems(true);
	}
}
...
void SourceTreeModel::Remove(obs_sceneitem_t *item)
{
	...
	if (items.count() == 0)
		GlobalSignal::instance()->onSigHaveSourceItems(false);
...
void SourceTree::ResetWidgets()
{
	...
	for (int i = 0; i < stm->items.count(); i++) {
		...
		SourceTreeItem *item = new SourceTreeItem(this, stm->items[i]);
		connect(item, SIGNAL(onSigSourceHandler(OBSSceneItem, int)), this, SLOT(onSourceHandler(OBSSceneItem, int)));
		setIndexWidget(index, item);

		emit onSigCreateSourceItem(stm->items[i]);
	}
}

void SourceTree::UpdateWidget(const QModelIndex &idx, obs_sceneitem_t *item)
{
	SourceTreeItem *sourceTreeItem = new SourceTreeItem(this, item);
	connect(sourceTreeItem, SIGNAL(onSigSourceHandler(OBSSceneItem, int)), this, SLOT(onSourceHandler(OBSSceneItem, int)));
	setIndexWidget(idx, sourceTreeItem);
}
...
void SourceTree::onSourceHandler(OBSSceneItem sceneitem, int type)
{
	SourceTreeModel *stm = GetStm();
	for (auto& it : stm->items) {
		auto bSelecetd = false;
		if (it == sceneitem) {
			bSelecetd = true;
		}
		SelectItem(it, bSelecetd);
	}

	switch (type)
	{
	case SOURCEITEM_SETTINGS:
		GlobalSignal::instance()->onSigSourceSet();
		break;
	case SOURCEITEM_UP:
		GlobalSignal::instance()->onSigSourceUp();
		break;
	case SOURCEITEM_DOWN:
		GlobalSignal::instance()->onSigSourceDown();
		break;
	case SOURCEITEM_DEL:
		GlobalSignal::instance()->onSigSourceDel();
		break;
	default:
		break;
	}
