...
void OBSBasic::SetCurrentScene(OBSSource scene, bool force, bool direct)
{
	...
	//obs_scene_t *sceneSource = obs_scene_from_source(scene);
	//obs_sceneitem_t *sceneItem = obs_scene_find_source(sceneSource, "logo_water_mark");
	//obs_source_t *source = NULL;
	//if (sceneItem == NULL)
	//{
	//	//m_pWaterMark->Add("./images/logo.png", "image_source", sceneSource);
	//	source = m_pWaterMark->Add("./images/logo.png", "image_source");
	//	//m_pWaterMark->ResetBackgroudLayer(source);
	//}
	//else
	//{
	...
	//if (g_dataCentre.stUserInformation.nShowRoomNumber != 0)
	//{
