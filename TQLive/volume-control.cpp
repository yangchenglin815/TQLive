...
void VolControl::setMicValue(int val)
{
	if (isAudioVolFlag) {
		slider->setValue(val);
	}
...
void VolControl::setJblValue(int val)
{
	if (!isAudioVolFlag) {
		slider->setValue(val);
	}
...
bool VolControl::isAudioVol()
{
	return isAudioVolFlag;
}
...
VolControl::VolControl(OBSSource source_, bool showConfig, bool vertical)
		...
		vertical      (vertical),
	    isAudioVolFlag(false)
{
	nameLabel = new QLabel();
	volLabel  = new QLabel();
	mute      = new MuteCheckBox();
	QString sourceName = obs_source_get_name(source);
	if (sourceName.contains(QStringLiteral("Âó¿Ë·ç/Aux"))) {
		isAudioVolFlag = true;
	}
	...
	if (isAudioVolFlag) {
		slider->setValue(UserConfig::instance()->getCurMICValue());
	}
	else {
		slider->setValue(UserConfig::instance()->getCurJBLValue());
	}
}
