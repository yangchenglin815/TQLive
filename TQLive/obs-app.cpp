...
OBSApp::OBSApp(int argc, char **argv)
	: /*QtSingleApplication("FFC66BD5-EBE4-4E2C-8E74-F0BD66EBAEA0", argc, argv)*/QApplication(argc, argv)/*QtSingleCoreApplication(argc, argv)*/
	//, m_singleInstance(argc, argv)
{
	qRegisterMetaType<EN_MSG_TYPE>("EN_MSG_TYPE");
}

void OBSApp::InitIM()
{
	connect((QObject *)CTaskNotification::GetInstance(), SIGNAL(signalIMLogout()), this, SLOT(slotIMLogout()));
}


OBSApp::~OBSApp()
{
	if (m_enExitStatusCode == kExitRejected)
	{
		return;
	}
...
void OBSApp::AppInit()
{
	...
	//QString qsStyleSheet = "{ font: bold italic large \"Times New Roman\" }";
...
void OBSApp::slotIMLogout()
{
	this->quit();
}

bool OBSApp::OBSInit(const QRect &rc)
{
	...
	//CIMMainDlg *pIMMainDlg = new CIMMainDlg();
	//connect(this, SIGNAL(destroyed()), qApp, SLOT(quit()));
	//pIMMainDlg->setAttribute(Qt::WA_DeleteOnClose, true);
	//pIMMainDlg->show();

	// solution A
	////CTQMainWidget *pTQLiveWindow = new CTQMainWidget(QApplication::desktop());			// 无法适配obs主窗口QMainWindow的情况，其父窗口不能为QWidget
	////pTQLiveWindow->resize(1060, 690);

	//CTQLiveWindow *pTQLiveWindow = new CTQLiveWindow();
	//pTQLiveWindow->setAttribute(Qt::WA_DeleteOnClose, true);
	//pTQLiveWindow->resize(1060, 690);

	////pTQLiveWindow->setResizeable(true);													// 无法两个QMainWindow的情况

	//
	////CCustomHeadBar *chb_head_bar = new CCustomHeadBar(NULL);
	////CCustomHeadBar *chb_head_bar = new CCustomHeadBar(pTQLiveWindow);
	//CCustomHeadBar *chb_head_bar = new CCustomHeadBar(pTQLiveWindow->ui.centralWidget);
	//chb_head_bar->setMinimumSize(QSize(1060, 60));
	//chb_head_bar->setMaximumSize(QSize(16777215, 60));
	//chb_head_bar->SetHostWindow(pTQLiveWindow);
	//chb_head_bar->SetBkColor(QColor(47, 47, 47, 255));

	////pTQLiveWindow->ui.verticalLayout->setSpacing(0);
	////pTQLiveWindow->ui.verticalLayout->setContentsMargins(0, 5, 5, 5);
	//pTQLiveWindow->ui.verticalLayout->insertWidget(0, chb_head_bar);
	//
	//QHBoxLayout *pHLayout = new QHBoxLayout();

	//int nMPWidth = 260, nMPHeight = 600;
	//int nMDWidth = 800, nMDHeight = 600;

	////CMainPanel *pMainPanel = new CMainPanel(pTQLiveWindow);
	//CMainPanel *pMainPanel = new CMainPanel(pTQLiveWindow->ui.centralWidget);
	//
	//pMainPanel->setFixedSize(nMPWidth, nMPHeight);
	////pMainPanel->resize(nMPWidth, nMPHeight);

	//pHLayout->addWidget(pMainPanel);

	////mainWindow = new OBSBasic(NULL);
	//mainWindow = new OBSBasic(pTQLiveWindow);
	//mainWindow->resize(nMDWidth, 600);


	////QWidget *parent = mainWindow->nativeParentWidget();
	//mainWindow->setAttribute(Qt::WA_DeleteOnClose, true);
	//connect(mainWindow, SIGNAL(destroyed()), this, SLOT(quit()));
	//pHLayout->addWidget(mainWindow);
	...
	//pTQLiveWindow->ui.verticalLayout->insertLayout(1, pHLayout);
	...
	//	pTQLiveWindow->move((nDCRWidth - nMPWidth - nMDWidth) / 2, (nDCRHeight - nMPHeight) / 2);
	//}
	...
	// solution B
	int nAppWindowWidth = 1060, nAppWindowHeight = 690;

	mainWindow = new OBSBasic(pDesktop);
	mainWindow->OBSInit();
	//this->setActivationWindow(mainWindow);

	//QWidget *pActiveWindow = this->activationWindow();

	//mainWindow->setStyleSheet(".QWidget { background-color: rgba(47, 47, 47, 255);}");
	...
	mainWindow->resize(nAppWindowWidth, nAppWindowHeight);

	if (NULL != pDesktop)
	{
		int nDCRWidth = pDesktop->availableGeometry().width();
		int nDCRHeight = pDesktop->availableGeometry().height();
		mainWindow->move((nDCRWidth - nAppWindowWidth) / 2, (nDCRHeight - nAppWindowHeight) / 2);
	}

	mainWindow->show();

	// 直播内容选择页面
	//CLiveContentSelectionDlg dlg(false, mainWindow.data());
	CLiveContentSelectionDlg dlg(true, mainWindow.data());
	//connect(&dlg, &CLiveContentSelectionDlg::signalLiveContentUpdate, (OBSBasic *)mainWindow.data(), &OBSBasic::slotUpdateLiveContent);
	connect(&dlg, SIGNAL(signalLiveContentUpdate(ST_MATCH_INFO &)), (OBSBasic *)mainWindow.data(), SLOT(slotUpdateLiveContent(ST_MATCH_INFO &)));
	dlg.resize(650, 430);
	dlg.exec();
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	if (!dir.exists(ExePath + "/image")) {
		dir.mkdir(ExePath + "/image");
	}

	//if (program.m_singleInstance.isRunning())
	//{
	//	return 0;
	//}

	//program.m_singleInstance.initialize();
	//QObject::connect(&program.m_singleInstance, SIGNAL(messageReceived(QString)), &program, SLOT(activateMainWidget()));
	...
	//if (program.sendMessage("instance"))
	//{
	//	return 0;
	//}

	//QObject::connect(&program, SIGNAL(messageReceived(QString)), &program, SLOT(slotMessageReceived(QString)));

	//program.initialize();

	//QObject::connect(&program, SIGNAL(messageReceived(QString)), &program, SLOT(activateMainWidget()));
	...
	QFont font(qsFamily, 13, 400);
	QApplication::setFont(qsFamily);
	...
	program.SetProfilerNameStore(profilerNameStore.get());
	program.SetSleepInhibitor();
	...
	qtranslator.load("qt_zh_CN.qm");
	program.installTranslator(&qtranslator);
	...
	if (!multi) {
		TipsDlg dlg(DIALOG_INSTANCE_EXIST_OPERATOR, nullptr);
		dlg.exec();
// 		QMessageBox::StandardButtons buttons(QMessageBox::Yes);
// 		QMessageBox mb(QMessageBox::Question, QString::fromLocal8Bit("探球直播助手"), QString::fromLocal8Bit("探球直播助手正在运行。"), buttons, nullptr);
// 		mb.setButtonText(QMessageBox::Yes, QString::fromLocal8Bit("确定"));
...
// 		button = (QMessageBox::StandardButton)mb.exec();
	...
	//bool bLoadUserInfo = true;

	//if (argc == 2)
	//{
	//	if (0 == strcmp(argv[1], "--invalidation"))
	//	{
	//		bLoadUserInfo = false;
	//	}
	//}

	//CLoginDlg dlg(NULL, bLoadUserInfo);
	//if (QDialog::Rejected == dlg.exec())
	//{
	//	program.SetExitStatusCode(kExitRejected);
	//	CController::GetInstance().PostQuitTask();
	//	return 0;
	//}
	...
	////CMainPanel *pMainPanel = new CMainPanel();

	//int nMPWidth = 400, nMPHeight = 760;
	//int nMDWidth = 1100, nMDHeight = 760;

	////pMainPanel->resize(nWidth, nHeight);
	//pMainPanel->setFixedSize(nMPWidth, nMPHeight);
	...
	////	int nDCRWidth = pDesktop->availableGeometry().width();
	////	int nDCRHeight = pDesktop->availableGeometry().height();
	////	pMainPanel->move((nDCRWidth - nMPWidth - nMDWidth) / 2, (nDCRHeight - nMPHeight) / 2);
	////}

	////pMainPanel->show();

	//int x = pMainPanel->x(), y = pMainPanel->y();
	////pMainPanel->move(x, y);

	//QSize size = pMainPanel->size();
	//QRect rc;
	//rc.setX(x + size.width());
	//rc.setY(y);
	//rc.setWidth(nMDWidth);
	//rc.setHeight(nMDHeight);
...
//	program.SetSleepInhibitor();
	...
	run:
		...
		if (argc == 2)
		{
			if (0 == strcmp(argv[1], "--invalidation") || 0 == strcmp(argv[1], "--logout"))
			{
				bLoadUserInfo = false;
			}
		}
		...
		CLoginDlg dlg(NULL, bLoadUserInfo);
		//program.setActivationWindow(&dlg);
		if (QDialog::Rejected == dlg.exec())
		{
			//program.SetExitStatusCode(kExitRejected);
			CController::GetInstance().PostQuitTask();
		...
		if (!program.OBSInit(/*rc*/))
			return 0;

		prof.Stop();

		int nResult = program.exec();

		CController::GetInstance().PostQuitTask(); // App对象销毁前退出工作线程
...
{
	CController::GetInstance().SetCrashReport();
	...
	//CController::GetInstance().PostQuitTask();
	CController::GetInstance().QuitIM();
	//CController::GetInstance().PostQuitIMTask();
...
void OBSApp::slotMessageReceived(QString qsMsg)
{
	qDebug("receive message from another instance.");
	QMainWindow *pMainWindow = this->GetMainWindow();
