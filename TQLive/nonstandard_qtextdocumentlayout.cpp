...
#include <algorithm>
#include "nonstandard_qtextdocumentlayout_p.h"
...
void QTextDocumentLayoutPrivate::drawFrameDecoration(QPainter *painter, QTextFrame *frame, QTextFrameData *fd, const QRectF &clip, const QRectF &rect) const
{
	...
	do {
		...
		if (_p_varProperty.isValid() == false) 
		{
			auto draw = [&]() {
				...
				{/*获得真正的宽度*/
					...
					for (; varP != varE; ++varP)
					{
						...
						for (int i = 0; i < varLineCount; ++i)
						{
							...
							if (varLineWidth > varMaxWidth)
							{
								varMaxWidth = varLineWidth;
							}
						}
					}
				...
				//varRect.moveTo(QPointF(varRect.x() + 40, varRect.y()));
...
//#include "moc_nonstandard_qtextdocumentlayout_p.cpp"
